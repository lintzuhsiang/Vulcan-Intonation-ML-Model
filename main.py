...
print("train data path, test data path, result csv filename")
#if feature numpy exists, don't need to calculate features again
...
else: train_x,train_y = data_process.train_data(sys.argv[1])

data_x= data_process.test_data(sys.argv[2])

#split training dataset and testing dataset 7:3
...
# Get our video data result from model
ans = []
...
second = 0
with open('%s.csv'%sys.argv[3],'w') as csv_file:
